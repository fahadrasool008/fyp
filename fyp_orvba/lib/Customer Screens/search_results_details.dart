...
class _SearchResultDetailsState extends State<SearchResultDetails> {
  ...
  void getRequest() async{
    ...
    print("current request ${myRequest}");
    print("current Customer ${currentCustomer}");
    print("current reviews ${myReviewsList}");
    ...
    if(myRequest == null){
      ...
      if(email == myRequest!["customerEmail"] && widget.service!["id"] == myRequest!["serviceId"]){
        setState(() {
          enabled = false;
        });
      }else{
  ...
  void createRequest() async {
    ...
    FirebaseDatabase.instance.ref("requests").push().set(requestData);
  }
  ...
  void writeReview(){
    ...
    _databaseReference.child("reviews").push().set(reviewsList).then((_){
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Reviewed Successfully")));
    });
      ...
      setState(() {
        ...
        if (event.snapshot.value != null) {
          ...
          if (data != null) {
            data.forEach((key, value) {
              value['id'] = key;
              reviewsList.add(value.cast<String, dynamic>()); // Cast to the correct type
            });
          }
        }
      });
          ...
          child: Column(
            children: [
              Expanded(
                ...
                child: ListView(
                  children: [
                    Image(image: NetworkImage(widget.service!["thumbnail"]), fit: BoxFit.cover,),
                    const SizedBox(
                      height: 5,
                    ),
                    Text(
                    ...
                    Row(
                      ...
                      children: [
                        Text("Rs. ${widget.service!["minPrice"]} - ${widget.service!["maxPrice"]}", style: bold16Red),
                        const Gap(50),
                        Text("${widget.service!["distance"].toStringAsFixed(0)} km", style: bold13Black)
                      ],
                    ),
                    const Divider(),
                    Column(
                      ...
                      children: [
                        ...
                        const Gap(10),
                        Text(widget.service!["description"], style: bold13Black,)
                      ],
                    ),
                    const Divider(),
                    if(!enabled)
                          ...
                          ListView.builder(
                            ...
                            physics: const NeverScrollableScrollPhysics(),
                                    ...
                                    itemBuilder: (context, index){
                                      Map<String,dynamic> item = myReviewsList[index];
                                      return ReviewContainer(value: item["rating"].toDouble(), customerName: item["customerName"], comment: item["comment"]);
                                    }),
                          ...
                          children: [
                            SizedBox(
                                ...
                                child: Padding(
                                  ...
                                  child: Text("Review this service",style: bold13Black,),

                                )),
                            CustomTextField(controller: commentController,keyoardType: TextInputType.text,),
                            Gap(20),
                            RatingStars(
                              value: rating,
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: GestureDetector(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(builder: (context)=> LocationDetails(isCustomer: true,latLng: LatLng(widget.service!["lat"], widget.service!["lon"]),)));
                          },
                          child: SmoothButton(
